...
class TestUsageCommand:
    """Tests for the /usage command handler in bot.py."""

    async def test_unauthorized_user_ignored(self):
        from bot import usage_command
    ...
    async def test_missing_config_sends_message(self):
        ...
        with (
            patch("bot.is_authorized", return_value=True),
            patch("bot.CLAUDE_SESSION_KEY", ""),
            patch("bot.CLAUDE_ORG_ID", ""),
        ):
            await usage_command(update, ctx)
        update.message.reply_text.assert_called_once()
        assert "not configured" in update.message.reply_text.call_args[0][0]

    async def test_session_expired_401(self):
        from bot import usage_command
        ...
        with (
            patch("bot.is_authorized", return_value=True),
            patch("bot.CLAUDE_SESSION_KEY", "sk-test"),
            patch("bot.CLAUDE_ORG_ID", "org-test"),
            patch("aiohttp.ClientSession", return_value=mock_session),
        ):
            await usage_command(update, ctx)
        update.message.reply_text.assert_called_once()
        assert "expired" in update.message.reply_text.call_args[0][0]

    async def test_successful_response_formatted(self):
        from bot import usage_command

        usage_data = {
            "five_hour": {"utilization": 23.5, "resets_at": "2026-05-06T12:00:00Z"},
            "seven_day": {"utilization": 61.0, "resets_at": "2026-05-10T00:00:00Z"},
            "seven_day_sonnet": {"utilization": 45.0, "resets_at": "2026-05-10T00:00:00Z"},
        }
