"""UK railway station CRS codes. Data from National Rail (~2600 stations)."""
...
STATIONS: dict[str, str] = {
    ...
    "HWY": "High Wycombe",
    "HXM": "Hoveton & Wroxham",
    "HXX": "Heathrow Terminals 1-2-3 Rail",
    "HYB": "Honeybourne",
    "HYC": "Hyde Central",
    "HYD": "Heyford",
    "HYH": "Hythe",
...
# Reverse lookup: lowercase station name -> CRS code
_NAMES_TO_CRS: dict[str, str] = {name.lower(): crs for crs, name in STATIONS.items()}


def search_stations(query: str, max_results: int = 20) -> list[dict[str, str]]:
    """Search for stations by name or CRS code (case-insensitive substring match)."""
    ...
    if q in STATIONS:
        results.append({"station_name": STATIONS[q], "crs_code": q})
    # Then substring matches on station name
    ql = query.strip().lower()
    for crs, name in STATIONS.items():
        if crs == q and results:
            continue  # Already added as exact match
        if ql in name.lower() or q == crs:
            results.append({"station_name": name, "crs_code": crs})
